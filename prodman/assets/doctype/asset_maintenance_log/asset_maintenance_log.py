...
from nts .utils import getdate, nowdate, today
...
class AssetMaintenanceLog(Document):
	...
	def validate(self):
		...
		if self.maintenance_status == "Completed" and not self.completion_date:
			nts .throw(_("Please select Completion Date for Completed Asset Maintenance Log"))

		if self.maintenance_status != "Completed" and self.completion_date:
			nts .throw(_("Please select Maintenance Status as Completed or remove Completion Date"))

	def on_submit(self):
		if self.maintenance_status not in ["Completed", "Cancelled"]:
			nts .throw(_("Maintenance Status has to be Cancelled or Completed to Submit"))
		self.update_maintenance_task()

	def update_maintenance_task(self):
		asset_maintenance_doc = nts .get_doc("Asset Maintenance Task", self.task)
		...
		if self.maintenance_status == "Cancelled":
			...
			asset_maintenance_doc.save()
		asset_maintenance_doc = nts .get_doc("Asset Maintenance", self.asset_maintenance)
		asset_maintenance_doc.save()
...
def update_asset_maintenance_log_status():
	...
	(
		nts .qb.update(AssetMaintenanceLog)
...
def get_maintenance_tasks(doctype, txt, searchfield, start, page_len, filters):
	asset_maintenance_tasks = nts .db.get_values(
