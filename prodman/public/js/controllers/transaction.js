...
prodman.TransactionController = class TransactionController extends prodman.taxes_and_totals {
	setup() {
		super.setup();
		let me = this;
		...
		if (this.frm.fields_dict["items"].grid.get_field("uom")) {
			this.frm.set_query("uom", "items", function(doc, cdt, cdn) {
				...
				return {
					query: "prodman.controllers.queries.get_item_uom_query",
					filters: {
		...
		if (this.frm.fields_dict["items"].grid.get_field('blanket_order')) {
			this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
				...
				return {
					query: "prodman.controllers.queries.get_blanket_orders",
					filters: {
						"company": doc.company,
	...
	send_sms() {
		var sms_man = new prodman.SMSManager(this.frm.doc);
	}
	...
	company() {
		...
		if (frappe.meta.get_docfield(this.frm.doctype, "shipping_address") &&
			['Purchase Order', 'Purchase Receipt', 'Purchase Invoice'].includes(this.frm.doctype)) {
				...
				if (!is_drop_ship) {
					prodman.utils.get_shipping_address(this.frm, function() {
		...
		if(this.frm.doc.company) {
			prodman.last_selected_company = this.frm.doc.company;
		}
	}
	...
	get_company_currency() {
		return prodman.get_currency(this.frm.doc.company);
	}

	contact_person() {
		prodman.utils.get_contact_details(this.frm);
	}
	...
	conversion_rate() {
		...
		this.frm.set_df_property("conversion_rate", "read_only", prodman.stale_rate_allowed() ? 0 : 1);
	}
	...
	ignore_pricing_rule() {
		if(this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "prodman.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	remove_pricing_rule_for_item(item) {
		...
		if (item.pricing_rules){
			let me = this;
			return this.frm.call({
				method: "prodman.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rule_for_item",
				args: {
	...
	apply_pricing_rule(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "prodman.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		// `prodman.stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm.call({
			method: "prodman.stock.get_item_details.apply_price_list",
			args: {	args: args, doc: me.frm.doc },
			callback: function(r) {
				if (!r.exc) {
					frappe.run_serially([
	...
	get_terms() {
		...
		prodman.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function(r) {
			if(!r.exc) {
	...
	update_item_tax_map() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "prodman.stock.get_item_details.get_item_tax_info",
				args: {
					company: me.frm.doc.company,
	...
	item_tax_template(doc, cdt, cdn) {
		...
		if(item.item_tax_template) {
			return this.frm.call({
				method: "prodman.stock.get_item_details.get_item_tax_map",
				args: {
					company: me.frm.doc.company,
					item_tax_template: item.item_tax_template,
					as_json: true
	...
	setup_item_selector() {
		...
		// 	this.item_selector = new prodman.ItemSelector({frm: this.frm});
		// }
	}
	...
	get_method_for_payment() {
		var method = "prodman.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
		if(cur_frm.doc.__onload && cur_frm.doc.__onload.make_payment_via_journal_entry){
			if(['Sales Invoice', 'Purchase Invoice'].includes( cur_frm.doc.doctype)){
				method = "prodman.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			}else {
				method= "prodman.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_item_tax_template(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			...
			return {
				query: "prodman.controllers.queries.get_tax_template",
				filters: filters
			}
		}
	}
...
prodman.show_serial_batch_selector = function (frm, item_row, callback, on_close, show_dialog) {
	...
	new prodman.SerialBatchPackageSelector(frm, item_row, (r) => {
		if (r) {
			let update_values = {
