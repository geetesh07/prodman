...
frappe.ui.form.on("Payment Entry", {
	...
	setup: function (frm) {
		...
		frm.set_query("payment_term", "references", function (frm, cdt, cdn) {
			...
			if (
				...
				return {
					query: "prodman.controllers.queries.get_payment_terms_for_references",
					filters: {
	...
	refresh: function (frm) {
		...
		prodman.accounts.ledger_preview.show_accounting_ledger_preview(frm);
		if (
			frm.doc.references &&
	...
	mode_of_payment: function (frm) {
		prodman.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
	...
	source_exchange_rate: function (frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", prodman.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function (frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", prodman.stale_rate_allowed() ? 0 : 1);
	},
