...
prodman.utils.get_party_details = function (frm, method, args, callback) {
	if (!method) {
...
prodman.utils.get_address_display = function (frm, address_field, display_field, is_your_company_address) {
...
prodman.utils.set_taxes_from_address = function (
	frm,
		...
		callback: function (r) {
			if (!r.exc) {
				if (frm.doc.tax_category != r.message) {
					...
					prodman.utils.set_taxes(frm, triggered_from_field);
				}
			}
		},
	});
...
prodman.utils.get_contact_details = function (frm) {
	if (frm.updating_party_details) return;
...
prodman.utils.validate_mandatory = function (frm, label, value, trigger_on) {
	if (!value) {
		frm.doc[trigger_on] = "";
...
prodman.utils.get_shipping_address = function (frm, callback) {
	if (frm.doc.company) {
		if (
