...
class QualityProcedure(NestedSet):
	...
	if TYPE_CHECKING:
		...
		from prodman.quality_management.doctype.quality_procedure_process.quality_procedure_process import (
	...
	def check_for_incorrect_child(self):
		for process in self.processes:
			if process.procedure:
				...
				parent_quality_procedure = nts.db.get_value(
				...
				if parent_quality_procedure and parent_quality_procedure != self.name:
					nts.throw(
						_("{0} already has a Parent Procedure {1}.").format(
							nts.bold(process.procedure), nts.bold(parent_quality_procedure)
						),
	...
	def add_child_to_parent(self):
		...
		if self.parent_quality_procedure:
			parent = nts.get_doc("Quality Procedure", self.parent_quality_procedure)
		...
		if old_doc := self.get_doc_before_save():
			if old_parent := old_doc.parent_quality_procedure:
				if self.parent_quality_procedure != old_parent:
					parent = nts.get_doc("Quality Procedure", old_parent)
...
def get_children(doctype, parent=None, parent_quality_procedure=None, is_root=False):
	...
	if parent:
		parent_procedure = nts.get_doc("Quality Procedure", parent)
		# return the list in order
		return [
			dict(
				value=d.procedure,
				expandable=nts.db.get_value("Quality Procedure", d.procedure, "is_group"),
			)
...
def add_node():
	...
	return nts.get_doc(args).insert()
