...
def execute():
	...
	nts.db.MAX_WRITES_PER_TRANSACTION += len(broken_sles)
	...
	for sle in broken_sles:
		...
		nts.db.set_value("Stock Ledger Entry", sle.name, "serial_no", correct_sr_no, update_modified=False)
	...
	broken_sr_no_records = nts.get_list(
		"Serial No",
		filters={
			"status": "Active",
	...
	nts.db.MAX_WRITES_PER_TRANSACTION += len(broken_sr_no_records)

	patch_savepoint = "serial_no_patch"
	for serial_no in broken_sr_no_records:
		try:
			nts.db.savepoint(patch_savepoint)
			sn = nts.get_doc("Serial No", serial_no)
			sn.update_serial_no_reference()
			sn.db_update()
		except Exception:
			nts.db.rollback(save_point=patch_savepoint)
