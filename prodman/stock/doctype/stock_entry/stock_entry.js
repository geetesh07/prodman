...
frappe.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		frm.set_query("batch_no", "items", function (doc, cdt, cdn) {
			...
			if (!item.item_code) {
				...
				return {
					query: "prodman.controllers.queries.get_batch_no",
					filters: filters,
				};
			}
		});
	...
	setup_quality_inspection: function (frm) {
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "prodman.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
		...
		if (frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				function () {
					prodman.utils.map_current_doc({
						method: "prodman.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = prodman.utils.map_current_doc({
	...
	toggle_display_account_head: function (frm) {
		var enabled = prodman.is_perpetual_inventory_enabled(frm.doc.company);
	...
	source_warehouse_address: function (frm) {
		prodman.utils.get_address_display(frm, "source_warehouse_address", "source_address_display", false);
	},

	target_warehouse_address: function (frm) {
		prodman.utils.get_address_display(frm, "target_warehouse_address", "target_address_display", false);
	},
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) prodman.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			prodman.utils.map_current_doc({
				method: "prodman.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
frappe.ui.form.on("Stock Entry Detail", {
	...
	expense_account(frm, cdt, cdn) {
		prodman.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},

	cost_center(frm, cdt, cdn) {
		prodman.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		prodman.stock.select_batch_and_serial_no(frm, child);
	},
});
...
prodman.stock.StockEntry = class StockEntry extends prodman.stock.StockController {
	setup() {
		var me = this;
		...
		this.frm.fields_dict.items.grid.get_field("item_code").get_query = function () {
			return prodman.queries.item({ is_stock_item: 1 });
		};
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 1 && prodman.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		prodman.hide_company(this.frm);
		prodman.utils.add_item(this.frm);
	}
	...
	set_default_account(company_fieldname, fieldname) {
		...
		return this.frm.call({
			method: "prodman.accounts.utils.get_company_default",
			args: {
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						prodman.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	toggle_related_fields(doc) {
		...
		if (prodman.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		prodman.utils.get_party_details(this.frm, null, null, null);
	}
};
...
prodman.stock.is_subcontracting_or_return_transfer = (doc) => {
...
function attach_bom_items(bom_no) {
	...
	frappe.db.get_doc("BOM", bom_no).then((bom) => {
		...
		items.forEach((item) => {
			prodman.stock.bom.items[item.item_code] = item;
		});
	});
}

function check_should_not_attach_bom_items(bom_no) {
	return bom_no === undefined || (prodman.stock.bom && prodman.stock.bom.name === bom_no);
}

extend_cscript(cur_frm.cscript, new prodman.stock.StockEntry({ frm: cur_frm }));
