...
nts.ui.form.on("Pick List", {
	...
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "prodman.stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
					company: frm.doc.company,
				},
			};
				...
				nts.prompt(
					...
					(data) => {
						...
						prodman.utils.map_current_doc({
							method: "prodman.manufacturing.doctype.work_order.work_order.create_pick_list",
							target: frm,
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new prodman.utils.BarcodeScanner(opts);
