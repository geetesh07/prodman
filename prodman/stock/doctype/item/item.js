...
frappe.ui.form.on("Item", {
	...
	onload: function (frm) {
		prodman.item.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		if (frm.doc.has_variants) {
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						prodman.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						prodman.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						prodman.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		prodman.item.edit_prices_button(frm);
		prodman.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			prodman.item.make_dashboard(frm);
		}
	...
	validate: function (frm) {
		prodman.item.weight_to_validate(frm);
	},
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				prodman.item.item_dashboard = new prodman.stock.ItemDashboard({
					...
					item_code: frm.doc.name,
					page_length: 20,
					method: "prodman.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				prodman.item.item_dashboard.refresh();
			});
		}
	},
...
frappe.tour["Item"] = [
	...
	},
	{
		...
		description: __(
			"If you are maintaining stock of this Item in your Inventory, prodman will make a stock ledger entry for each transaction of this item."
		),
	},
	{
