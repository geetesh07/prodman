...
import nts 
from nts  import _, msgprint, qb, scrub
from nts .contacts.doctype.address.address import get_company_address, get_default_address
...
from nts .query_builder.functions import Abs, Count, Date, Sum
...
from prodman.exceptions import InvalidAccountCurrency, PartyDisabled, PartyFrozen
from prodman.utilities.regional import temporary_flag

try:
	from nts .contacts.doctype.address.address import render_address as _render_address
except ImportError:
	# Older nts  versions where this function is not available
	from nts .contacts.doctype.address.address import get_address_display as _render_address
...
class DuplicatePartyAccountError(nts .ValidationError):
	pass
...
def get_party_details(
	...
	if not nts .db.exists(party_type, party):
		nts .throw(_("{0}: {1} does not exists").format(party_type, party))
...
def _get_party_details(
	...
	party = nts .get_doc(party_type, party)

	if not ignore_permissions:
		ptype = "select" if nts .only_has_select_perm(party_type) else "read"
		nts .has_permission(party_type, ptype, party, throw=True)
	...
	if party_type == "Supplier" and party:
		party_details["supplier_tds"] = nts .get_value(party_type, party.name, "tax_withholding_category")

	if not party_details.get("tax_category") and pos_profile:
		party_details["tax_category"] = nts .get_value("POS Profile", pos_profile, "tax_category")
...
def complete_contact_details(party_details):
	contact_details = nts ._dict()

	if party_details.party_type == "Employee":
		contact_details = nts .db.get_value(
			"Employee",
			party_details.party,
			[
...
def get_default_price_list(party):
	...
	if party.doctype == "Customer":
		return nts .get_cached_value("Customer Group", party.customer_group, "default_price_list")
...
def set_price_list(party_details, party, party_type, given_price_list, pos=None):
	...
	if price_list and len(price_list) == 1:
		...
		customer_price_list = nts .get_value("Customer", party.name, "default_price_list")

		if customer_price_list:
			price_list = customer_price_list
		else:
			pos_price_list = nts .get_value("POS Profile", pos, "selling_price_list")
			price_list = pos_price_list or given_price_list
	else:
		price_list = get_default_price_list(party) or given_price_list

	if price_list:
		party_details.price_list_currency = nts .db.get_value(
...
def get_party_account(party_type, party=None, company=None, include_advance=False):
	...
	if not party and party_type in ["Customer", "Supplier"]:
		...
		return nts .get_cached_value("Company", company, default_account_name)
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		group = nts .get_cached_value(party_type, party, scrub(party_group_doctype))
		account = nts .db.get_value(
			"Party Account",
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		account = nts .get_cached_value("Company", company, default_account_name)
	...
	if not account:
		account_type = nts .get_cached_value("Party Type", party_type, "account_type")
...
def get_party_bank_account(party_type, party):
	return nts .db.get_value("Bank Account", {"party_type": party_type, "party": party, "is_default": 1})


def get_party_account_currency(party_type, party, company):
	def generator():
		party_account = get_party_account(party_type, party, company)
		return nts .get_cached_value("Account", party_account, "account_currency")

	return nts .local_cache("party_account_currency", (party_type, party, company), generator)
...
def get_party_gle_account(party_type, party, company):
	def generator():
		existing_gle_account = nts .db.sql(
	...
	if existing_gle_currency and party_account_currency != existing_gle_currency:
		nts .throw(
			_(
				...
				nts .bold(party_type),
				nts .bold(party),
				nts .bold(existing_gle_currency),
				nts .bold(company),
			),
...
def validate_party_accounts(doc):
	from prodman.controllers.accounts_controller import validate_account_head
	...
	for account in doc.get("accounts"):
		...
		party_account_currency = nts .get_cached_value("Account", account.account, "account_currency")
		if nts .db.get_default("Company"):
			company_default_currency = nts .get_cached_value(
				"Company", nts .db.get_default("Company"), "default_currency"
			)
		else:
			company_default_currency = nts .get_cached_value("Company", account.company, "default_currency")
	...
	if (bill_date or posting_date) and party:
		...
		if template_name:
			...
			if party_type == "Supplier":
				supplier_group = nts .get_cached_value(party_type, party, "supplier_group")
				template_name = nts .get_cached_value("Supplier Group", supplier_group, "payment_terms")
				if template_name:
...
def get_due_date_from_template(template_name, posting_date, bill_date):
	...
	template = nts .get_doc("Payment Terms Template", template_name)
...
def validate_due_date(posting_date, due_date, bill_date=None, template_name=None, doctype=None):
	if getdate(due_date) < getdate(posting_date):
		nts .throw(_("Due Date cannot be before Posting / Supplier Invoice Date"))
	else:
...
def validate_due_date_with_template(posting_date, due_date, bill_date, template_name, doctype=None):
	...
	if default_due_date != posting_date and getdate(due_date) > getdate(default_due_date):
		if nts .db.get_single_value("Accounts Settings", "credit_controller") in nts .get_roles():
			...
			nts .throw(_("Due / Reference Date cannot be after {0}").format(formatdate(default_due_date)))


@nts .whitelist()
def get_address_tax_category(tax_category=None, billing_address=None, shipping_address=None):
	addr_tax_category_from = nts .db.get_single_value(
		"Accounts Settings", "determine_address_tax_category_from"
	)
	if addr_tax_category_from == "Shipping Address":
		if shipping_address:
			tax_category = nts .db.get_value("Address", shipping_address, "tax_category") or tax_category
	else:
		if billing_address:
			tax_category = nts .db.get_value("Address", billing_address, "tax_category") or tax_category
...
def set_taxes(
	...
	from prodman.accounts.doctype.tax_rule.tax_rule import get_party_details, get_tax_template

	args = {nts .scrub(party_type): party, "company": company}
...
def get_payment_terms_template(party_name, party_type, company=None):
	...
	if party_type == "Customer":
		customer = nts .get_cached_value(
			"Customer", party_name, fieldname=["payment_terms", "customer_group"], as_dict=1
		)
		template = customer.payment_terms

		if not template and customer.customer_group:
			template = nts .get_cached_value("Customer Group", customer.customer_group, "payment_terms")
	else:
		supplier = nts .get_cached_value(
			"Supplier", party_name, fieldname=["payment_terms", "supplier_group"], as_dict=1
		)
		template = supplier.payment_terms
		if not template and supplier.supplier_group:
			template = nts .get_cached_value("Supplier Group", supplier.supplier_group, "payment_terms")

	if not template and company:
		template = nts .get_cached_value("Company", company, fieldname="payment_terms")
	return template


def validate_party_frozen_disabled(party_type, party_name):
	if nts .flags.ignore_party_validation:
		return

	if party_type and party_name:
		if party_type in ("Customer", "Supplier"):
			party = nts .get_cached_value(party_type, party_name, ["is_frozen", "disabled"], as_dict=True)
			if party.disabled:
				nts .throw(_("{0} {1} is disabled").format(party_type, party_name), PartyDisabled)
			elif party.get("is_frozen"):
				frozen_accounts_modifier = nts .db.get_single_value(
					"Accounts Settings", "frozen_accounts_modifier"
				)
				if frozen_accounts_modifier not in nts .get_roles():
					nts .throw(_("{0} {1} is frozen").format(party_type, party_name), PartyFrozen)

		elif party_type == "Employee":
			if nts .db.get_value("Employee", party_name, "status") != "Active":
				nts .msgprint(_("{0} {1} is not active").format(party_type, party_name), alert=True)
...
def validate_account_party_type(self):
	...
	if self.party_type and self.party:
		account_type = nts .get_cached_value("Account", self.account, "account_type")
...
def get_dashboard_info(party_type, party, loyalty_program=None):
	...
	company_wise_grand_total = nts .get_all(
		doctype,
		filters={
			"docstatus": 1,
			party_type.lower(): party,
	...
	if party_type == "Customer":
		loyalty_point_details = nts ._dict(
			nts .get_all(
				"Loyalty Point Entry",
				filters={
					"customer": party,
	...
	company_wise_billing_this_year = nts ._dict()
	...
	company_wise_total_unpaid = nts ._dict(
		nts .db.sql(
			"""
	...
	for d in companies:
		company_default_currency = nts .get_cached_value("Company", d.company, "default_currency")
...
def get_party_shipping_address(doctype: str, name: str) -> str | None:
	...
	:return: String
	"""
	shipping_addresses = nts .get_all(
		"Address",
		filters=[
...
def get_partywise_advanced_payment_amount(
	...
	if invoice_doctypes := nts .get_hooks("invoice_doctypes"):
...
def add_party_account(party_type, party, company, account):
	doc = nts .get_doc(party_type, party)
...
def render_address(address, check_permissions=True):
	return nts .call(_render_address, address, check_permissions=check_permissions)
