...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from prodman.stock.doctype.item_default.item_default import ItemDefault
		from prodman.stock.doctype.item_reorder.item_reorder import ItemReorder
		from prodman.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from prodman.stock.doctype.item_tax.item_tax import ItemTax
		from prodman.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from prodman.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_prodman_to_barcodenumber(
...
def convert_prodman_to_barcodenumber(prodman_number, barcode):
	if prodman_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if prodman_number in convert:
		return convert[prodman_number]
