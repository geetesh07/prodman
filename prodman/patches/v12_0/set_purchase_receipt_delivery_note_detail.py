...
def execute():
	nts.reload_doc("stock", "doctype", "delivery_note_item", force=True)
	nts.reload_doc("stock", "doctype", "purchase_receipt_item", force=True)
	...
	def make_return_document_map(doctype, return_document_map):
		...
		return_against_documents = nts.db.sql(
			f"""
			SELECT
	...
	def set_document_detail_in_return_document(doctype):
		...
		child_doc = nts.scrub(f"{doctype} Item")
		nts.reload_doc("stock", "doctype", child_doc)
		...
		for docname in return_document_map:
			doc_items = nts.get_cached_doc(doctype, docname).get("items")
			for return_doc in return_document_map[docname]:
				return_doc_items = nts.get_cached_doc(doctype, return_doc).get("items")
