...
prodman.sales_common = {
	setup_selling_controller: function () {
		prodman.selling.SellingController = class SellingController extends prodman.TransactionController {
			setup() {
				super.setup();
			...
			onload() {
				...
				this.frm.set_query("project", function (doc) {
					return {
						query: "prodman.controllers.queries.get_project_name",
						filters: {
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], prodman.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", prodman.queries.contact_query);
				me.frm.set_query("company_contact_person", prodman.queries.company_contact_query);
				me.frm.set_query("customer_address", prodman.queries.address_query);
				me.frm.set_query("shipping_address_name", prodman.queries.address_query);
				me.frm.set_query("dispatch_address_name", prodman.queries.dispatch_address_query);
				me.frm.set_query("company_address", prodman.queries.company_address_query);
			...
			customer_address() {
				prodman.utils.get_address_display(this.frm, "customer_address");
				prodman.utils.set_taxes_from_address(
					this.frm,
					"customer_address",
					"customer_address",
			...
			shipping_address_name() {
				prodman.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				prodman.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				prodman.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			pick_serial_and_batch(doc, cdt, cdn) {
				...
				frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
					if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
						...
						new prodman.SerialBatchPackageSelector(me.frm, item, (r) => {
							if (r) {
