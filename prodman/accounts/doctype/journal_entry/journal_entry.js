...
nts .ui.form.on("Journal Entry", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1) {
			frm.add_custom_button(
				...
				function () {
					return prodman.journal_entry.reverse_journal_entry(frm);
				},
				__("Actions")
			);
		}
		...
		if (frm.doc.__islocal) {
			frm.add_custom_button(__("Quick Entry"), function () {
				return prodman.journal_entry.quick_entry(frm);
			});
		}

		// hide /unhide fields based on currency
		prodman.journal_entry.toggle_fields_based_on_currency(frm);
	...
	posting_date: function (frm) {
		...
		$.each(frm.doc.accounts || [], function (i, row) {
			prodman.journal_entry.set_exchange_rate(frm, row.doctype, row.name);
		});
	},
...
prodman.accounts.JournalEntry = class JournalEntry extends nts .ui.form.Controller {
	...
	setup_queries() {
		...
		me.frm.set_query("account", "accounts", function (doc, cdt, cdn) {
			return prodman.journal_entry.account_query(me.frm);
		});
	...
	accounts_add(doc, cdt, cdn) {
		...
		prodman.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "accounts");
	}
};

cur_frm.script_manager.make(prodman.accounts.JournalEntry);
...
nts .ui.form.on("Journal Entry Account", {
	...
	account: function (frm, dt, dn) {
		prodman.journal_entry.set_account_details(frm, dt, dn);
	},

	debit_in_account_currency: function (frm, cdt, cdn) {
		prodman.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
		...
		var dialog = new nts .ui.Dialog({
			...
			fields: [
				{ fieldtype: "Currency", fieldname: "debit", label: __("Amount"), reqd: 1 },
				{
					...
					options: "Account",
					get_query: function () {
						return prodman.journal_entry.account_query(frm);
					},
				},
				{
					fieldtype: "Link",
		...
		if (d.account) {
			...
			return nts .call({
				...
				callback: function (r) {
					if (r.message) {
						$.extend(d, r.message);
						prodman.journal_entry.set_amount_on_last_row(frm, dt, dn);
						prodman.journal_entry.set_debit_credit_in_company_currency(frm, dt, dn);
						refresh_field("accounts");
					}
				},
			});
