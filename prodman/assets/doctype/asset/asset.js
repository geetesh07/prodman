...
nts .ui.form.on("Asset", {
	...
	setup: function (frm) {
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				query: "prodman.controllers.queries.get_purchase_receipts",
				filters: { item_code: doc.item_code },
			};
		});
		frm.set_query("purchase_invoice", (doc) => {
			return {
				query: "prodman.controllers.queries.get_purchase_invoices",
				filters: { item_code: doc.item_code },
			};
		});
	},
		...
		if (frm.doc.docstatus == 1) {
			if (["Submitted", "Partially Depreciated", "Fully Depreciated"].includes(frm.doc.status)) {
				frm.add_custom_button(
					__("Transfer Asset"),
					function () {
						prodman.asset.transfer_asset(frm);
					},
					__("Manage")
				);
				...
				frm.add_custom_button(
					__("Restore Asset"),
					function () {
						prodman.asset.restore_asset(frm);
					},
					__("Manage")
				);
			}
...
prodman.asset.scrap_asset = function (frm) {
	nts .confirm(__("Do you really want to scrap this asset?"), function () {
		nts .call({
			...
			method: "prodman.assets.doctype.asset.depreciation.scrap_asset",
			callback: function (r) {
				cur_frm.reload_doc();
			},
		});
...
prodman.asset.restore_asset = function (frm) {
	nts .confirm(__("Do you really want to restore this scrapped asset?"), function () {
		nts .call({
			...
			method: "prodman.assets.doctype.asset.depreciation.restore_asset",
			callback: function (r) {
				cur_frm.reload_doc();
			},
		});
