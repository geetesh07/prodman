...
nts.ui.form.on("Sales Order", {
	...
	onload: function (frm) {
		...
		frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "prodman.controllers.queries.warehouse_query";
		...
		const dialog = new nts.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					get_query: () => {
						return {
							query: "prodman.controllers.queries.get_filtered_child_rows",
							filters: {
...
nts.ui.form.on("Sales Order Item", {
	...
	delivery_date: function (frm, cdt, cdn) {
		if (!frm.doc.delivery_date) {
			prodman.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	},
});

prodman.selling.SalesOrderController = class SalesOrderController extends prodman.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
		...
		if (this.frm.doc.docstatus === 0 && nts.model.can_read("Quotation")) {
			this.frm.add_custom_button(
				__("Quotation"),
				function () {
					let d = prodman.utils.map_current_doc({
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "prodman.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new nts.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "prodman.selling.doctype.sales_order.sales_order.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
...
extend_cscript(cur_frm.cscript, new prodman.selling.SalesOrderController({ frm: cur_frm }));
