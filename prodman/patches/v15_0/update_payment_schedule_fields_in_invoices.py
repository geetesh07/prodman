...
def execute():
	...
	for invoice_type in invoice_types:
		...
		invoice_details = nts.qb.from_(invoice).select(invoice.conversion_rate, invoice.name)
...
def update_payment_schedule(invoice_details):
	...
	nts.qb.update(ps).join(invoice_details).on(ps.parent == invoice_details.name).set(
