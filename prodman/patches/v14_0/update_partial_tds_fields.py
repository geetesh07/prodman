...
from prodman.accounts.utils import FiscalYearError, get_fiscal_year
...
def execute():
	...
	for company in nts.get_all("Company"):
		try:
			...
			nts.qb.update(purchase_invoice).set(
			...
			purchase_order = nts.qb.DocType("Purchase Order")

			nts.qb.update(purchase_order).set(
