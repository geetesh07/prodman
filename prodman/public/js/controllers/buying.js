...
prodman.buying = {
	setup_buying_controller: function() {
		prodman.buying.BuyingController = class BuyingController extends prodman.TransactionController {
			setup() {
				super.setup();
				...
				if(this.frm.get_field('shipping_address')) {
					this.frm.set_query("shipping_address", () => {
						if(this.frm.doc.customer) {
							...
							return prodman.queries.company_address_query(this.frm.doc)
					});
				}

				if(this.frm.get_field('dispatch_address')) {
					this.frm.set_query("dispatch_address", () => {
						return prodman.queries.address_query(this.frm.doc);
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query('supplier', prodman.queries.supplier);
				me.frm.set_query('contact_person', prodman.queries.contact_query);
				me.frm.set_query('supplier_address', prodman.queries.address_query);

				me.frm.set_query('billing_address', prodman.queries.company_address_query);
				prodman.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);
				...
				this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
					...
					return {
						query: "prodman.controllers.queries.item_manufacturer_query",
			...
			supplier() {
				var me = this;
				prodman.utils.get_party_details(this.frm, null, null, function(){
					me.apply_price_list();
				});
			}
			...
			supplier_address() {
				prodman.utils.get_address_display(this.frm);
				prodman.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
			}
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(__("Product Bundle"), function() {
					prodman.buying.get_items_from_product_bundle(me.frm);
				}, __("Get Items From"));
			}

			shipping_address(){
				var me = this;
				prodman.utils.get_address_display(this.frm, "shipping_address",
					"shipping_address_display", true);
			}

			dispatch_address(){
				var me = this;
				prodman.utils.get_address_display(this.frm, "dispatch_address",
					"dispatch_address_display", true);
			}

			billing_address() {
				prodman.utils.get_address_display(this.frm, "billing_address",
				...
				frappe.db.get_value("Item", item.item_code, fields)
					.then((r) => {
						if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
							...
							new prodman.SerialBatchPackageSelector(
								me.frm, item, (r) => {
									if (r) {
