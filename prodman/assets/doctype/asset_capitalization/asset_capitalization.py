...
class AssetCapitalization(StockController):
	...
	if TYPE_CHECKING:
		...
		from prodman.assets.doctype.asset_capitalization_service_item.asset_capitalization_service_item import (
			AssetCapitalizationServiceItem,
		)
		from prodman.assets.doctype.asset_capitalization_stock_item.asset_capitalization_stock_item import (
	...
	def validate_item(self, item):
		from prodman.stock.doctype.item.item import validate_end_of_life
		...
		for item_row in self.stock_items:
			...
			if sle_list:
				for sle in sle_list:
					...
					if prodman.is_perpetual_inventory_enabled(self.company):
