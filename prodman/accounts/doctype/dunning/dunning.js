...
nts .ui.form.on("Dunning", {
	setup: function (frm) {
		...
		frm.set_query("company_address", prodman.queries.company_address_query);
	...
	conversion_rate: function (frm) {
		if (frm.doc.currency === prodman.get_currency(frm.doc.company)) {
			frm.set_value("conversion_rate", 1.0);
		}

		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("conversion_rate", "read_only", prodman.stale_rate_allowed() ? 0 : 1);
	},
	customer_address: function (frm) {
		prodman.utils.get_address_display(frm, "customer_address");
	},
	company_address: function (frm) {
		prodman.utils.get_address_display(frm, "company_address");
	},
