...
from nts .query_builder.functions import Max, Min
...
from prodman.accounts.doctype.journal_entry.journal_entry import make_reverse_journal_entry
from prodman.assets.doctype.asset_activity.asset_activity import add_asset_activity
...
def post_depreciation_entries(date=None):
	...
	if not cint(
		nts .db.get_value("Accounts Settings", None, "book_asset_depreciation_entry_automatically")
	):
		return
...
def get_depreciable_asset_depr_schedules_data(date):
	a = nts .qb.DocType("Asset")
	ads = nts .qb.DocType("Asset Depreciation Schedule")
...
def get_acc_frozen_upto():
	...
	frozen_accounts_modifier = nts .db.get_single_value("Accounts Settings", "frozen_accounts_modifier")

	if frozen_accounts_modifier not in nts .get_roles() or nts .session.user == "Administrator":
...
def make_depreciation_entry(
	...
	nts .has_permission("Journal Entry", throw=True)

	if not date:
		date = today()

	asset_depr_schedule_doc = nts .get_doc("Asset Depreciation Schedule", asset_depr_schedule_name)

	asset = nts .get_doc("Asset", asset_depr_schedule_doc.asset)
...
def get_credit_and_debit_accounts(accumulated_depreciation_account, depreciation_expense_account):
	root_type = nts .get_value("Account", depreciation_expense_account, "root_type")
	...
	if root_type == "Expense":
		...
		nts .throw(_("Depreciation Expense Account should be an Income or Expense Account."))
...
def set_depr_entry_posting_status_for_failed_assets(failed_asset_names):
	for asset_name in failed_asset_names:
		nts .db.set_value("Asset", asset_name, "depr_entry_posting_status", "Failed")
...
def scrap_asset(asset_name):
	...
	if asset.docstatus != 1:
		nts .throw(_("Asset {0} must be submitted").format(asset.name))
	elif asset.status in ("Cancelled", "Sold", "Scrapped", "Capitalized"):
		nts .throw(_("Asset {0} cannot be scrapped, as it is already {1}").format(asset.name, asset.status))
	...
	depreciation_series = nts .get_cached_value("Company", asset.company, "series_for_depreciation_entry")
	...
	nts .db.set_value("Asset", asset_name, "disposal_date", date)
	nts .db.set_value("Asset", asset_name, "journal_entry_for_scrap", je.name)
	asset.set_status("Scrapped")

	add_asset_activity(asset_name, _("Asset scrapped"))

	nts .msgprint(_("Asset scrapped via Journal Entry {0}").format(je.name))
...
def modify_depreciation_schedule_for_asset_repairs(asset, notes):
	asset_repairs = nts .get_all(
		"Asset Repair", filters={"asset": asset.name}, fields=["name", "increase_in_asset_life"]
	)

	for repair in asset_repairs:
		if repair.increase_in_asset_life:
			asset_repair = nts .get_doc("Asset Repair", repair.name)
		...
		for schedule_idx, schedule in enumerate(asset_depr_schedule_doc.get("depreciation_schedule")):
			if schedule.schedule_date == date and schedule.journal_entry:
				if not disposal_was_made_on_original_schedule_date(
					...
					nts .flags.is_reverse_depr_entry = True
					reverse_journal_entry.submit()

					nts .flags.is_reverse_depr_entry = False
...
def get_depreciation_accounts(asset_category, company):
	...
	if not fixed_asset_account:
		nts .throw(_("Please set Fixed Asset Account in Asset Category {0}").format(asset_category))

	if not non_depreciable_category:
		accounts = nts .get_cached_value(
...
def get_disposal_account_and_cost_center(company):
	disposal_account, depreciation_cost_center = nts .get_cached_value(
		"Company", company, ["disposal_account", "depreciation_cost_center"]
	)

	if not disposal_account:
		nts .throw(_("Please set 'Gain/Loss Account on Asset Disposal' in Company {0}").format(company))
	if not depreciation_cost_center:
		nts .throw(_("Please set 'Asset Depreciation Cost Center' in Company {0}").format(company))
