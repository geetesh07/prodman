...
def execute():
	...
	"""
	work_order = nts.qb.DocType("Work Order")
	query = (
		nts.qb.from_(work_order)
		.select(
			work_order.name,
	...
	for row in results:
		so_item = nts.get_value("Material Request Item", row.material_request_item, "sales_order_item")
		nts.db.set_value("Work Order", row.name, "sales_order_item", so_item)

		if so_item:
			wo = nts.get_doc("Work Order", row.name)
