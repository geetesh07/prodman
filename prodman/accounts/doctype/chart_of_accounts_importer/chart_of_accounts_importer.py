...
import nts 
from nts  import _
from nts .desk.form.linked_with import get_linked_fields
...
def validate_columns(data):
	if not data:
		nts .throw(_("No data found. Seems like you uploaded a blank file"))
...
def validate_company(company):
	parent_company, allow_account_creation_against_child_company = nts .get_cached_value(
		"Company", company, ["parent_company", "allow_account_creation_against_child_company"]
	)

	if parent_company and (not allow_account_creation_against_child_company):
		msg = _("{} is a child company.").format(nts .bold(company)) + " "
		msg += _("Please import accounts against parent company or enable {} in company master.").format(
			nts .bold(_("Allow Account Creation Against Child Company"))
		)
		nts .throw(msg, title=_("Wrong Company"))

	if nts .db.get_all("GL Entry", {"company": company}, "name", limit=1):
		return False
...
def get_file(file_name):
	file_doc = nts .get_doc("File", {"file_url": file_name})
...
def generate_data_from_csv(file_doc, as_dict=False):
	...
	with open(file_path) as in_file:
		...
		for row in csv_reader:
			if as_dict:
				data.append({nts .scrub(header): row[index] for index, header in enumerate(headers)})
			else:
...
def build_forest(data):
	...
	if error_messages:
		nts .throw("<br>".join(error_messages))
...
def build_response_as_excel(writer):
	...
	nts .response["filename"] = "coa_importer_template.xlsx"
	nts .response["filecontent"] = xlsx_file.getvalue()
...
def download_template(file_type, template_type, company):
	...
	if file_type == "CSV":
		# download csv file
		nts .response["result"] = cstr(writer.getvalue())
		nts .response["type"] = "csv"
		nts .response["doctype"] = "Chart of Accounts Importer"
	else:
...
def get_sample_template(writer, company):
	currency = nts .db.get_value("Company", company, "default_currency")
...
def validate_accounts(file_doc, extension):
	...
	for account in accounts:
		...
		if "parent_account" not in account:
			...
			nts .throw(msg, title=_("Parent Account Missing"))
...
def validate_missing_roots(roots):
	...
	if missing:
		nts .throw(_("Please add Root Account for - {0}").format(" , ".join(missing)))
...
def unset_existing_data(company):
	...
	nts .db.set_value("Company", company, update_values, update_values)
...
def set_default_accounts(company):
	...
	company.update(
		{
			"default_receivable_account": nts .db.get_value(
				"Account", {"company": company.name, "account_type": "Receivable", "is_group": 0}
			),
			"default_payable_account": nts .db.get_value(
				"Account", {"company": company.name, "account_type": "Payable", "is_group": 0}
			),
			"default_provisional_account": nts .db.get_value(
				"Account",
