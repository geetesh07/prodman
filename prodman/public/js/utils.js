...
$.extend(prodman, {
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (prodman.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", prodman.last_selected_company);
			}
		}
	},
...
$.extend(prodman.utils, {
	set_party_dashboard_indicators: function (frm) {
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					prodman.utils.add_indicator_for_multicompany(frm, info);
				});
...
prodman.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
			...
			{ fieldtype: "Section Break", label: __("Items") },
			{
				...
				fields: [
					...
					},
					{
						...
						get_query: (e) => {
							return {
								query: "prodman.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
...
prodman.utils.update_child_items = function (opts) {
	const frm = opts.frm;
...
prodman.utils.map_current_doc = function (opts) {
	function _map() {
