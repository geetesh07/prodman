...
frappe.ui.form.on("Work Order", {
	setup: function (frm) {
		...
		frm.set_query("operation", "required_items", function () {
			return {
				query: "prodman.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
	...
	onload: function (frm) {
		...
		if (frm.doc.__islocal) {
			...
			prodman.work_order.set_default_warehouse(frm);
		}
	},
	...
	refresh: function (frm) {
		...
		prodman.work_order.set_custom_buttons(frm);
		frm.set_intro("");
			...
			frappe.call({
				...
				callback: function (r) {
					if (r.message) {
						...
						prodman.in_production_item_onchange = true;
						...
						prodman.in_production_item_onchange = false;
					}
				},
			});
		}
	...
	project: function (frm) {
		if (!prodman.in_production_item_onchange && !frm.doc.bom_no) {
		...
		if (doc.docstatus === 1 && !["Closed", "Completed"].includes(doc.status)) {
			if (doc.status != "Stopped" && doc.status != "Completed") {
				frm.add_custom_button(
					__("Stop"),
					function () {
						prodman.work_order.change_work_order_status(frm, "Stopped");
					},
					__("Status")
				);
				...
				frm.add_custom_button(
					__("Re-open"),
					function () {
						prodman.work_order.change_work_order_status(frm, "Resumed");
					},
					__("Status")
				);
			}
			...
			if (show_start_btn) {
				...
				if (pending_to_transfer && frm.doc.status != "Stopped") {
					...
					frm.add_custom_button(__("Create Pick List"), function () {
						prodman.work_order.create_pick_list(frm);
					});
					var start_btn = frm.add_custom_button(__("Start"), function () {
						prodman.work_order.make_se(frm, "Material Transfer for Manufacture");
					});
						...
						if (counter > 0) {
							var consumption_btn = frm.add_custom_button(
								...
								function () {
									...
									prodman.work_order.make_consumption_se(
										frm,
					...
					if (flt(doc.material_transferred_for_manufacturing) > 0) {
						if (flt(doc.produced_qty) < flt(doc.material_transferred_for_manufacturing)) {
							...
							let finish_btn = frm.add_custom_button(__("Finish"), function () {
								prodman.work_order.make_se(frm, "Manufacture");
							});
